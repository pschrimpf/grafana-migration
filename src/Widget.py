...
class Widget:
    ...
    def __init__(self, conversionService, widget):
        ...
        try:
            ...
            elif self.panelType == 'heatmap':
                self.visualisation = "viz.heatmap"
            ...
            if self.panelType == 'text':
                ...
                # Make content convertable
                content = GrafanaHelper.normalizeQuery(content)
                content = re.sub(r'width=\".+?\"', "", content)
                content = re.sub(r'height=\".+?\"', "", content)
                content = html_to_markdown.convert_to_markdown(content)
        ...
        for target in targets:
            ...
            if 'expr' in target:
                queries.append({
                    ...
                    "query": self.conversionService.convertPromQLQuery(target['expr'], range=range)
                })
